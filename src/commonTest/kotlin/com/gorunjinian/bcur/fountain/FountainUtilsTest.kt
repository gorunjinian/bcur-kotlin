...
class FountainUtilsTest {
    @Test
    fun testShuffle() {
        val rng = Xoshiro256StarStar("Wolf")
        val numbers = (1..10).toMutableList()
        val shuffled = FountainUtils.shuffled(numbers, rng)
        assertEquals(listOf(6, 4, 9, 3, 10, 5, 7, 8, 1, 2), shuffled)
    }
    ...
    fun testXOR() {
        ...
        rng.nextData(data1)
        assertEquals("916ec65cf77cadf55cd7", TestUtils.bytesToHex(data1))

        val data2 = ByteArray(10)
        rng.nextData(data2)
        assertEquals("f9cda1a1030026ddd42e", TestUtils.bytesToHex(data2))

        val data3 = FountainEncoder.xor(data1, data2)
        assertEquals("68a367fdf47c8b2888f9", TestUtils.bytesToHex(data3))
    }

    @Test
    fun testPartitionAndJoin() {
        val message = TestUtils.makeMessage(1024, "Wolf")
        val fragmentLen = FountainEncoder.findNominalFragmentLength(message.size, 10, 100)
        val fragments = FountainEncoder.partitionMessage(message, fragmentLen)
        val fragmentsHex = fragments.map { TestUtils.bytesToHex(it) }
        ...
        assertEquals(expected, fragmentsHex)

        val rejoined = FountainDecoder.joinFragments(fragments, message.size)
        assertContentEquals(message, rejoined)
    }
}
